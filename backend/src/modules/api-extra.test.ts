...
describe("assignments endpoints", () => {
  ...
  beforeAll(async () => {
    ...
    classId = await createClass(
      `CLS-ASGE-${Date.now().toString().slice(-6)}`,
      guruId,
    );
    ...
    assignmentId = await createAssignment(
      promptId,
      classId,
      `Ujian-ASGE-${Date.now().toString().slice(-6)}`,
    );
  }, 30000);
  ...
  it("guru nonaktifkan ujian → 200, siswa tidak lagi melihatnya", async () => {
    ...
    expect(
      avail.body.data.some((a: { id: number }) => a.id === assignmentId),
    ).toBe(false);
  });
...
describe("prompts endpoints", () => {
  ...
  it("detail soal → 200 + data lengkap", async () => {
    ...
    const res = await request(app)
      .get(`/api/v1/prompts/${createdPrompt}`)
      .set("Authorization", `Bearer ${guruToken}`);
    expect(res.status).toBe(200);
  ...
  it("hapus soal → 200, lalu 404", async () => {
    const del = await request(app)
      .delete(`/api/v1/prompts/${createdPrompt}`)
      .set("Authorization", `Bearer ${guruToken}`);
...
describe("admin class/users endpoints", () => {
  ...
  it("list users → 200", async () => {
    ...
    expect(
      res.body.data.some(
        (u: { email: string }) => u.email === "admin@pessay.test",
      ),
    ).toBe(true);
  });
...
describe("review flow via API", () => {
  it("guru review submission needs_review → graded dengan skor", async () => {
    ...
    const cls = await createClass(
      `CLS-RVF-${Date.now().toString().slice(-6)}`,
      guru.id,
    );
    ...
    const asg = await createAssignment(
      prompt,
      cls,
      `Ujian-RVF-${Date.now().toString().slice(-6)}`,
    );
    const sub = await createSubmission(
      asg,
      siswa.id,
      "Perlu tinjauan manual guru.",
      "needs_review",
    );
