...
describe('submissions flow', () => {
  beforeAll(async () => {
    ...
    classId = await createClass(`CLS-SUB-${Date.now().toString().slice(-6)}`, guruId);
    await enroll(classId, siswaId);
    promptId = await createPrompt({ subject: 'bahasa' });
    assignmentId = await createAssignment(promptId, classId, `Ujian-SUB-${Date.now().toString().slice(-6)}`);
  }, 30000);
  ...
  it('siswa submit → 202 accepted, status pending', async () => {
    const res = await request(app)
      ...
      .send({ assignmentId, body: 'Ini adalah jawaban esai saya tentang pentingnya pendidikan bagi generasi muda Indonesia yang cerdas dan berkarakter.' });
...
describe('review manual oleh guru', () => {
  ...
  beforeAll(async () => {
    ...
    guruToken = await login(guru.email, guru.password);
    siswaToken = await login(siswa.email, siswa.password);
    reviewClass = await createClass(`CLS-RV-${Date.now().toString().slice(-6)}`, guru.id);
    await enroll(reviewClass, siswa.id);
    reviewPrompt = await createPrompt({ subject: 'bahasa' });
    reviewAssignment = await createAssignment(reviewPrompt, reviewClass, `Ujian-RV-${Date.now().toString().slice(-6)}`);
    reviewSubmission = await createSubmission(reviewAssignment, siswa.id, 'Teks jawaban perlu ditinjau manual oleh guru.', 'needs_review');
  }, 30000);
...
describe('rate limit submit', () => {
  it('submit kedua dalam 10 detik → 429', async () => {
    ...
    const cls = await createClass(`CLS-RL-${Date.now().toString().slice(-6)}`, guru.id);
    await enroll(cls, siswa.id);
    const prompt = await createPrompt({ subject: 'bahasa' });
    const asg = await createAssignment(prompt, cls, `Ujian-RL-${Date.now().toString().slice(-6)}`);

    const first = await request(app)
      .post('/api/v1/submissions/submit')
      .set('Authorization', `Bearer ${token}`)
      .send({ assignmentId: asg, body: 'Jawaban pertama untuk menguji batas frekuensi pengiriman oleh siswa bersangkutan.' });
    expect(first.status).toBe(202);

    const second = await request(app)
      .post('/api/v1/submissions/submit')
      .set('Authorization', `Bearer ${token}`)
      .send({ assignmentId: asg, body: 'Jawaban kedua yang seharusnya ditolak oleh sistem pengendali frekuensi pengiriman yang ketat.' });
...
it('unauthorized tanpa token → 401', async () => {
  const res = await request(app).post('/api/v1/submissions/submit').send({ assignmentId, body: 'tanpa token' });
