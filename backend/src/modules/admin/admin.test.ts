...
describe("prompts", () => {
  ...
  it("guru buat soal matematika dengan bahasa latex → 201", async () => {
    const res = await request(app)
      ...
      .send({
        ...
        language: "id",
        instructions:
          "Selesaikan: \\(2x + 4 = 16\\). Tulis langkah pengerjaan.",
        rubric: [
          { name: "Langkah", max: 50, description: "Proses" },
          { name: "Jawaban Akhir", max: 50, description: "Hasil" },
        ],
        maxScore: 100,
      });
    expect(res.status).toBe(201);
  ...
  it("listing soal dengan filter subject", async () => {
    const res = await request(app)
      .get("/api/v1/prompts?subject=bahasa")
      .set("Authorization", `Bearer ${guruToken}`);
...
describe("assignments", () => {
  it("guru buat ujian untuk kelas → 201, siswa melihatnya di available", async () => {
    ...
    const classId = await createClass(
      `CLS-ASG-${Date.now().toString().slice(-6)}`,
      guru.id,
    );
    ...
    const create = await request(app)
      ...
      .send({
        promptId,
        classId,
        title: `Ujian-ASG-${Date.now().toString().slice(-6)}`,
      });
...
describe("admin", () => {
  ...
  it("admin buat kelas + enroll siswa", async () => {
    const create = await request(app)
      ...
      .set("Authorization", `Bearer ${adminToken}`)
      .send({
        code: `CLS-ADM-${Date.now().toString().slice(-6)}`,
        name: "Kelas Admin",
      });
    expect(create.status).toBe(201);
