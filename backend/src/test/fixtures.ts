...
export async function cleanupTestData(): Promise<void> {
  ...
  await pool.query(
    `DELETE FROM submissions WHERE student_id IN (SELECT id FROM users WHERE email LIKE $1)`,
    [like],
  );
  await pool.query(
  ...
  await pool.query(
    `DELETE FROM anomalies WHERE submission_id IN (SELECT s.id FROM submissions s JOIN users u ON u.id = s.student_id WHERE u.email LIKE $1)`,
    [like],
  );
  await pool.query(`DELETE FROM assignments WHERE title LIKE $1`, [
    `Ujian ${runToken()}%`,
  ]);
  await pool.query(
    `DELETE FROM enrollments WHERE class_id IN (SELECT id FROM classes WHERE code LIKE $1)`,
    [like],
  );
  await pool.query(`DELETE FROM classes WHERE code LIKE $1`, [like]);
  await pool.query(`DELETE FROM prompts WHERE title LIKE $1`, [
    `Soal ${runToken()} %`,
  ]);
