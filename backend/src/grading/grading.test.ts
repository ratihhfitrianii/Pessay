  ...
  it("menghasilkan hasil matematika dengan mathCorrect", async () => {
    const r = await adapter.grade({
      ...
      rubric: [
        ...
        { name: "Jawaban", max: 50, description: "Hasil akhir" },
      ],
      maxScore: 100,
    });
...
describe("batch grader (pipeline end-to-end)", () => {
  ...
  beforeAll(async () => {
    ...
    const { rows } = await pool.query(
      ...
      [
        `siswa-batch-${Date.now()}@pessay.test`,
  ...
  it("worker memproses submission pending → graded", async () => {
    ...
    const { rows } = await pool.query(
      `SELECT status, score FROM submissions WHERE id = $1`,
      [subId],
    );
    ...
    const scores = await pool.query(
      `SELECT dimension, score FROM scores WHERE submission_id = $1`,
      [subId],
    );
