...
export class GeminiGradingAdapter implements GradingAdapter {
  ...
  constructor(apiKey: string, model: string) {
    if (!apiKey)
      throw new AppError(
        "CONFIG_ERROR",
        "GEMINI_API_KEY wajib diisi saat GRADING_MODE=real",
        500,
      );
    this.apiKey = apiKey;
    this.model = model;
  ...
  async grade(input: GradingInput): Promise<GradingResult> {
    ...
    if (!res.ok) {
      ...
      throw new AppError(
        "AI_PROVIDER_ERROR",
        `Gemini API gagal (${res.status}): ${text.slice(0, 200)}`,
        502,
      );
    }
...
function normalizeResult(
  ...
  const clamp = (n: number, max: number) =>
    Math.max(0, Math.min(max, Math.round(n * 100) / 100));
...
export function createGradingAdapter(
  mode: string,
  apiKey: string,
  model: string,
): GradingAdapter {
