...
export class MockGradingAdapter implements GradingAdapter {
  ...
  async grade(input: GradingInput): Promise<GradingResult> {
    ...
    const vocabScore = words.length > 0 ? Math.min(1, uniq / Math.max(words.length, 1) / 0.7) : 0;

    const dims: RubricDimension[] =
      input.rubric.length > 0 ? input.rubric : [{ name: 'Kualitas', max: input.maxScore, description: '' }];

    const dimensions = dims.map((d, i) => {
      const base = [lengthScore, structureScore, vocabScore][i % 3] ?? 0.6;
      const score = Math.round((Math.max(0.35, Math.min(0.98, base)) * d.max) * 100) / 100;
      return {
        name: d.name,
        score,
        max: d.max,
    ...
    const overall = Math.round(dimensions.reduce((s, d) => s + d.score, 0) * 100) / 100;
    const confidence = Math.round(Math.min(0.95, 0.5 + lengthScore * 0.4) * 100) / 100;
    ...
    if (input.subject === 'matematika') {
      ...
      return {
        ...
        dimensions,
        feedback: ['Langkah pengerjaan teridentifikasi.', 'Jawaban akhir diperiksa.'],
    ...
    return {
      ...
      overallScore: overall,
      dimensions,
      feedback: ['Teks memiliki struktur yang jelas.', 'Kosa kata cukup bervariasi.'],
