...
export async function processSubmission(
  ...
  try {
    ...
    if (
      integrity.unscorable ||
      integrity.anomalies.some((a) => a.type === "unscorable")
    ) {
      await client.query(
    ...
    await client.query(`DELETE FROM scores WHERE submission_id = $1`, [
      submissionId,
    ]);
    ...
    const needsReview =
      result.confidence < 0.5 || integrity.anomalies.length > 0;
    ...
    await client.query(
      ...
      [
        ...
        JSON.stringify({
          ...
          mathCorrect:
            result.subject === "matematika" ? result.mathCorrect : undefined,
          correctAnswer:
            result.subject === "matematika" ? result.correctAnswer : undefined,
        }),
