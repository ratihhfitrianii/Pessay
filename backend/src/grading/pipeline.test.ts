...
describe("pipeline processSubmission", () => {
  ...
  beforeAll(async () => {
    ...
    classId = await createClass(
      `CLS-PIPE-${Date.now().toString().slice(-6)}`,
      guruId,
    );
  ...
  it("submission pendek (unscorable) → status unscorable, tidak diproses AI", async () => {
    ...
    assignmentId = await createAssignment(
      promptId,
      classId,
      `Ujian-PIPE1-${Date.now().toString().slice(-6)}`,
    );
    ...
    const { rows } = await pool.query(
      `SELECT status, anomaly_json FROM submissions WHERE id = $1`,
      [subId],
    );
    expect(rows[0]!.status).toBe("unscorable");
    expect(JSON.parse(rows[0]!.anomaly_json)[0]).toHaveProperty(
      "type",
      "unscorable",
    );
  });
  ...
  it("submission normal → graded dengan score > 0", async () => {
    ...
    assignmentId = await createAssignment(
      promptId,
      classId,
      `Ujian-PIPE2-${Date.now().toString().slice(-6)}`,
    );
    const subId = await createSubmission(
      assignmentId,
      siswaId,
    ...
    const scores = await pool.query(
      `SELECT dimension FROM scores WHERE submission_id = $1`,
      [subId],
    );
  ...
  it("submission not found → 404 error", async () => {
    await expect(processSubmission(999999, adapter)).rejects.toThrow(
      /tidak ditemukan/,
    );
  });
  ...
  it("submission status graded (bukan pending) → return status tanpa proses", async () => {
    ...
    assignmentId = await createAssignment(
      promptId,
      classId,
      `Ujian-PIPE3-${Date.now().toString().slice(-6)}`,
    );
    const subId = await createSubmission(
      assignmentId,
      siswaId,
  ...
  it("adapter throw → error diteruskan, status kembali pending (rollback)", async () => {
    ...
    assignmentId = await createAssignment(
      promptId,
      classId,
      `Ujian-PIPE4-${Date.now().toString().slice(-6)}`,
    );
    const subId = await createSubmission(
      assignmentId,
      siswaId,
    ...
    await expect(processSubmission(subId, failing as never)).rejects.toThrow(
      /provider error/,
    );
    const { rows } = await pool.query(
  ...
  it("rubric JSON rusak → safeParseRubric mengembalikan [] dan tetap graded", async () => {
    ...
    await pool.query(
      `UPDATE prompts SET rubric_json = '{rusak' WHERE id = $1`,
      [promptId],
    );
    assignmentId = await createAssignment(
      promptId,
      classId,
      `Ujian-PIPE5-${Date.now().toString().slice(-6)}`,
    );
    const subId = await createSubmission(
      assignmentId,
      siswaId,
