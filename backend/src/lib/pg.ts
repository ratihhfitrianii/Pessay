...
pool.on('error', (err) => {
  ...
  console.warn('[pg] koneksi idle ditutup, pool akan membuat koneksi baru:', err.message);
});
...
export async function pingDatabase(retries = 3, delayMs = 2000): Promise<void> {
  for (let i = 1; i <= retries; i++) {
    try {
      ...
      // eslint-disable-next-line no-console
      console.warn(`[pg] startup SELECT 1 gagal (${i}/${retries}) — mungkin cold start, mencoba lagi...`);
